...
def user(request):
    ...
    if request.method == "DELETE":
        ...
        try: 
            ...
            return Response({"message": "User deleted sucssesfully!"}, status=status.HTTP_200_OK)
        except:
            return Response({"message": "Filed to delete user!"}, status=status.HTTP_400_BAD_REQUEST)
...
def forgot_password(request):
    ...
    activation_link = f"{base_url}/reset-password/{uid}/{token}"

    send_mail(
        subject="Reset Your Password",
        message=f"Hi {user.username}, use the link below to reset your password: {activation_link}",
        from_email=settings.DEFAULT_FROM_EMAIL,
